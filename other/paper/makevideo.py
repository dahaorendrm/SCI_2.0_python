...
def matvideo(name = '4D_color_checker', filerange = (0,4), nframe = 24):
    if not os.path.exists('./resultpaper/exp/mat'):
       os.mkdir('./resultpaper/exp/mat')
    for idx in range(*filerange):
        mea =  tif.imread('/lustre/arce/X_MA/SCI_2.0_python/resultpaper/exp/S0/spvi/mea/'+name+f'{idx:04d}'+'.tiff')
        led =  tif.imread('/lustre/arce/X_MA/SCI_2.0_python/resultpaper/exp/S0/spvi/img_n/'+name+f'{idx:04d}'+'.tiff')
        led_complete =  tif.imread('/lustre/arce/X_MA/SCI_2.0_python/resultpaper/exp/S2/re/'+name+f'{idx:04d}'+'.tiff')
        img =  tif.imread('/lustre/arce/X_MA/SCI_2.0_python/resultpaper/exp/S3/result/'+name+f'{idx:04d}'+'.tiff.tiff')
        rgb = specimg2rgb(img)
        scio.savemat('./resultpaper/exp/mat/'+name+f'{idx:04d}'+'.mat',{'mea':mea,'rgb':rgb,'led':led,'spec':img,'led_complete':led_complete})

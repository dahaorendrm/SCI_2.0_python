...
if __name__=='__main__':
    if not os.path.exists('expdata'):
        os.mkdir('expdata')
    path = Path('../data/expdata_SCI_2.0/20210930data')
    ...
    for idx,img in enumerate(imgs):
        scio.savemat(f'expdata/4D_color_checker{idx:04d}.mat',{'img':img})
    imgs = converter(path,(41,80),1024,(3,1),0.25)
    for idx,img in enumerate(imgs):
        scio.savemat(f'expdata/4D_color_checker_motion{idx:04d}.mat',{'img':img})
    imgs = converter(path,(87,116),1024,(3,1),0.25)
    for idx,img in enumerate(imgs):
        scio.savemat(f'expdata/4D_rotor{idx:04d}.mat',{'img':img})
    imgs = converter(path,(317,366),1024,(3,1),0.25)
    for idx,img in enumerate(imgs):
        scio.savemat(f'expdata/4D_Lego{idx:04d}.mat',{'img':img})
    imgs = converter(path,(467,576),1024,(3,1),0.25)
    for idx,img in enumerate(imgs):
        scio.savemat(f'expdata/4D_Mango1_{idx:04d}.mat',{'img':img})
    imgs = converter(path,(567,616),1024,(3,1),0.25)
    for idx,img in enumerate(imgs):
        scio.savemat(f'expdata/4D_Mango2_{idx:04d}.mat',{'img':img})
